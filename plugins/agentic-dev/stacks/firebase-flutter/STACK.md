...
| [`stage-access.md`](stage-access.md) | **Pre-live, the loop never waits on a human for infrastructure** — the stage-1 provisioning package (self-served IAM repair, database creation, Firebase/Stripe MCP wiring), the two access lanes, and the backup concept |
